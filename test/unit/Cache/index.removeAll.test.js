describe('Cache#removeAll()', function () {
  it('should remove all items in the cache.', function () {
    var cache = TestCacheFactory('DSCache.removeAll.cache');
    var value1 = 'value1',
      value2 = 2,
      value3 = {
        value3: 'stuff'
  ...
  it('should remove items from localStorage when storageMode is used.', function () {
    var localStorageCache = TestCacheFactory('DSCache.removeAll.localStorageCache', { storageMode: 'localStorage', storageImpl: localStorage }),
      sessionStorageCache = TestCacheFactory('DSCache.removeAll.sessionStorageCache', { storageMode: 'sessionStorage', storageImpl: sessionStorage, storagePrefix: 'affas' });
    ...
    assert.equal(JSON.parse(localStorage.getItem(localStorageCache.$$storagePrefix + 'DSCache.removeAll.localStorageCache.data.item1')).value, 'value1');
    assert.equal(JSON.parse(localStorage.getItem(localStorageCache.$$storagePrefix + 'DSCache.removeAll.localStorageCache.data.item2')).value, 'value2');
    assert.equal(localStorage.getItem(localStorageCache.$$storagePrefix + 'DSCache.removeAll.localStorageCache.keys'), '["item1","item2"]');
    assert.equal(JSON.parse(sessionStorage.getItem(sessionStorageCache.$$storagePrefix + 'DSCache.removeAll.sessionStorageCache.data.item1')).value, 'value1');
    assert.equal(JSON.parse(sessionStorage.getItem(sessionStorageCache.$$storagePrefix + 'DSCache.removeAll.sessionStorageCache.data.item2')).value, 'value2');
